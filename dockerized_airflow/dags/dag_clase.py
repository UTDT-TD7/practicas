...
cwd = os.getcwd()
path=cwd.replace('/dags','/Includes')
...
column_names_2=['sessionid','iteminsession','userid','ts','auth','level','trackid',
                'song','artist','zip','city','state','useragent','lon','lat','lastname',
                'firstname','gender','registration', 'duration','fecha']


extract_query_1="SELECT * FROM listen_events OFFSET "+str(limit_1)+" ROWS FETCH NEXT 10 ROWS ONLY"
...
insert_query="INSERT INTO listen_events (" +names+", fecha"+") VALUES " +"("+symbol+")"
...
# In[] Fluxo
#Ok,ahora debo diseñar la estrategia de Airflow

default_args={"depends_on_past":True}


with DAG(dag_id="DAG_catedra_airflow",
    ...
    t1=PythonOperator(task_id="first-step-el",
                      python_callable=step.step_ingesta_carga_pura,
                      provide_context=True,
                      op_kwargs={'host_1':host_1,
                                 'database':database,
                                 'user':user,
                                 'password':password,
                                 'port':port,
    ...
    t2=PythonOperator(task_id="second-step-el",
                      python_callable=step.step_ingesta_transformacion_carga,
                      provide_context=True,
                      op_kwargs={'host':host_2,
                                 'database':database,
                                 'user':user,
                                 'password':password,
                                 'port':port,
